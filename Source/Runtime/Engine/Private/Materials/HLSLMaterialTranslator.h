...
ResourcesString = TEXT("");
	...
	void GetMaterialEnvironment(EShaderPlatform InPlatform, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		{	
			...
			//add MobileSkin shading model 编译着色器时为SGSSS添加类型描述宏
			if (ShadingModels.HasShadingModel(MSM_MobileSkin))
			{
				OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_MobileSkin"), TEXT("1"));
				NumSetMaterials++;
			}
